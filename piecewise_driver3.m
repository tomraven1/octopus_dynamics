function pos = piecewise_driver3(inp,tim,ini_cond)
format long
...
T11      =zeros(length(t_tens),1);%zeros(length(t_tens),1);%inp(4,:)'/4;% zeros(length(t_tens),1);
T12        =zeros(length(t_tens),1);
T13      =zeros(length(t_tens),1);%zeros(length(t_tens),1);%inp(2,:)'/4;% zeros(length(t_tens),1);

T21      =zeros(length(t_tens),1);%inp(4,:)';
T22      =zeros(length(t_tens),1);%inp(4,:)';
T23      =zeros(length(t_tens),1);%inp(4,:)';

T31      =inp(1,:)';
T32      =inp(2,:)';%inp(2,:)';%inp(3,:)';
T33      =inp(3,:)' ;%inp(3,:)'%zeros(length(t_tens),1) ;
...
E        =1*110e3;                         % [Pa] modulo di young 110e3
eta      =1*3e2;                           % [Pa*s] viscosità 3e3
Poi      =0.5;                           % [-] modulo di Poisson 0.5
G        =E/(2*(1+Poi));                 % [Pa] modulo di taglio
R1       =13.8e-3; %13.8e-3;                       % [m] Raggio sezione 1
R2       =13.8e-3; %11.1e-3;                       % [m] Raggio sezione 2
R3       =13.8e-3;%8.2e-3;                        % [m] Raggio sezione 3
R4       =13.8e-3; %5.4e-3;  
R        =[R1 R2 R3 R4];
...
ro_water    =1022;%1022;                                   % [Kg/m^3] densità
...
Fax         =[-T31-T32-T33 -T21-T22-T23 -T13-T11-T12...
...
Famy        =[-T32*d3 -T22*d2 -T12*d1...
    zeros(length(t_tens),1)]; % [Nm] bending torque
Famz        =[(T31-T33)*d3 (T21-T23)*d2 (T11-T13)*d1...
    zeros(length(t_tens),1)]; % [Nm] bending torque
%-------------------------------------------------------------------------
